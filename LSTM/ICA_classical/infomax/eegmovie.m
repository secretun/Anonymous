...
%              Uses scripts eegplot() and topoplot().
...
% 6/4/97 Colin Humphries & Scott Makeig, CNL / Salk Institute / La Jolla CA
%
...
eegplot('noui',-data,srate,0,eloc_locs,startsec,'r'); 
...
for i = movieframes                      % make the movie, frame by frame
   ...
   l1 = line([i i],limits,'color','b'); % draw vertical line at data frame
   ...
   Movie(:,i+1-movieframes(1)) = getframe(gcf);
   drawnow
   delete(l1)
